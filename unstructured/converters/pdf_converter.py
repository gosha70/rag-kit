...
from langchain_community.document_loaders import PyPDFLoader, MathpixPDFLoader
...
class PdfConverter(BaseFileConverter):
    ...
    def load_and_split_file(self, text_splitter: TextSplitter, file_path: str) -> List[Document]:
        ...
        """
        loader = self.get_pdf_loader(file_path)
        return loader.load_and_split()
    
    def get_pdf_loader(self, file_path: str) -> UnstructuredBaseLoader:
        """
        Returns the appropriate PDF loader based on the presence of a Mathpix API key.
        
        Parameters:
        - file_path (str): The path to the PDF file
        
        Returns:
        - Union[MathpixPDFLoader, UnstructuredPDFLoader]: The appropriate PDF loader
        """
        mathpix_api_key = os.environ.get('MATHPIX_API_KEY')
        if mathpix_api_key:
            try:
                self.log_info("Creating MathpixPDFLoader.")
                return MathpixPDFLoader(file_path, api_key=mathpix_api_key)
            except Exception as e:
                self.log_info(f"MathpixPDFLoader failed: {str(e)}. Falling back to PyPDFLoader.")
        
        self.log_info("Creating PyPDFLoader.")
